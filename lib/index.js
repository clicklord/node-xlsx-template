...
module.exports = (function () {
  ...
  Workbook.prototype.loadTemplate = function (data) {
    ...
    self.contentTypes = etree
      .parse(self.archive.file("[Content_Types].xml").asText())
      .getroot();
    var jpgType = self.contentTypes.find('Default[@Extension="jpg"]');
    if (jpgType === null) {
      etree.SubElement(self.contentTypes, "Default", {
    ...
    sheetData.findall("row").forEach(function (row) {
      ...
      if (newTableRows.length > 0) {
        ...
        function makeRowContinuous(r, row) {
          var newRow = self.cloneElement(r, true);
          var cols = [];
          r._children.forEach(function (c) {
              cols.push(self.splitRef(c.attrib.r).col);
          });
          for (var i = 0; i < row._children.length; i++) {
              var cell = row._children[i];
              if(cell.attrib.s !== undefined) {
                  var oCol = self.splitRef(cell.attrib.r).col;
                  if (!contains(cols, oCol)) {
                      var newCell = self.cloneElement(cell, false);
  ...
  Workbook.prototype.addContentType = function (partName, contentType) {
    var self = this;
    etree.SubElement(self.contentTypes, "Override", {
  ...
  Workbook.prototype.initDrawing = function (sheet, rels) {
    ...
    var drawingFilename =
      "drawing" +
      self.findMaxFileId(/xl\/drawings\/drawing\d*\.xml/, /drawing(\d*)\.xml/) +
      ".xml";
  ...
  Workbook.prototype.substituteImage = function (
    cell,
    string,
    placeholder,
    substitution,
    ...
    //get max refid
    //update rel file.
    var maxId = self.findMaxId(
      drawing.relRoot,
      "Relationship",
      "Id",
      /rId(\d*)/
    );
    var maxFildId = self.findMaxFileId(
    ...
    self.sheet.root
      .findall("mergeCells/mergeCell")
      .forEach(function (mergeCell) {
    ...
    fromCol.text = (
      self.charToNum(self.splitRef(cell.attrib.r).col) - 1
    ).toString();
    ...
    fromRowOff.text = "0";
    var extImagePart = etree.SubElement(imagePart, "xdr:ext", {
      cx: imageWidth,
      cy: imageHeight,
    });
    var picNode = etree.SubElement(imagePart, "xdr:pic");
    var nvPicPr = etree.SubElement(picNode, "xdr:nvPicPr");
    var cNvPr = etree.SubElement(nvPicPr, "xdr:cNvPr", {
      id: maxId,
      name: "image_" + maxId,
      descr: "",
    });
    var cNvPicPr = etree.SubElement(nvPicPr, "xdr:cNvPicPr");
    var picLocks = etree.SubElement(cNvPicPr, "a:picLocks", {
      noChangeAspect: "1",
    });
    ...
    var ext = etree.SubElement(xfrm, "a:ext", {
      cx: imageWidth,
      cy: imageHeight,
    });
  ...
  Workbook.prototype.getWidthCell = function (numCol, sheet) {
    var defaultWidth = sheet.root.find("sheetFormatPr").attrib[
      "defaultColWidth"
    ];
    if (!defaultWidth) {
  ...
  Workbook.prototype.getHeightCell = function (numRow, sheet) {
    var defaultHight = sheet.root.find("sheetFormatPr").attrib[
      "defaultRowHeight"
    ];
  ...
  Workbook.prototype.imageToBuffer = function (imageObj) {
    ...
    if (imageObj instanceof Buffer) {
      ...
      if (typeof imageObj === "string" || imageObj instanceof String) {
        ...
        //}else{
        if (
          "imageRootPath" in this.option &&
          fs.existsSync(this.option.imageRootPath + "/" + imageObj)
        ) {
          //get the Absolute path file
          return Buffer.from(
            fs.readFileSync(this.option.imageRootPath + "/" + imageObj, {
              encoding: "base64",
            }),
            "base64"
          );
          ...
          if (fs.existsSync(imageObj)) {
            ...
            return Buffer.from(
              fs.readFileSync(imageObj, { encoding: "base64" }),
              "base64"
            );
          }
        }
